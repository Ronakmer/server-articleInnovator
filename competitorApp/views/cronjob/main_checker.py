...
def send_article_url_to_create_article(competitor_domain_mapping_obj, article_url):
    try:
        ...
        data = {
            ...
            "url": article_url,
            "keyword": None,
            "author_slug_id": competitor_domain_mapping_obj.wp_author,
            "category_slug_id": wp_category.get('category_slug_id_ai', ''),
            "tag_slug_id": wp_tag.get('tag_slug_id_ai', ''),
            "ai_content_flags": competitor_domain_mapping_obj.ai_content_flags
        }

        # add_ai_article(request_user,data)
                        
        login_and_add_article(data)
...
def login_and_add_article(article_data):
    try:
        ...
        login_response = requests.post(login_url, data=login_payload, verify=ssl_verify)
        print("Login Status:", login_response.status_code)
        print("Login Raw Response:", repr(login_response.text))
        ...
        if not login_json.get("success"):
            print("Login failed:", login_json.get("message"))
            return False
        ...
        print("Add Article Status:", article_response.status_code)
        print("Add Article Raw Response:", repr(article_response.text))
        ...
        if article_response.status_code == 200 and article_json.get("success"):
            print("Article added successfully.")
            return True
        else:
            print("Article creation failed:", article_json)
            return False

    except Exception as e:
        print("Error during login and article creation:", e)
        return False
