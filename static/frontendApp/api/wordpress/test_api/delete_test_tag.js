...
async function delete_test_tag(test_tag_id) {
    ...
    console.log(data)
    try{
        const response = await fetch(delete_test_tag_url, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            // const deletetagLabel = document.getElementById('delete_test_tag_id');
            const loader = document.getElementById('loader5-spinner');
                
            if (data.status_code === 200 || data.status_code === 201) {
                // deletetagLabel.innerText = 'Success';
