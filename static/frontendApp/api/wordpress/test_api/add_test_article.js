...
// Add Test Article
async function add_test_article() {
    const access_token = sessionStorage.getItem("access_token");

    try{
        const response = await fetch(add_test_article_url, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            const loader = document.getElementById('loader2-spinner');
            if (data.status_code === 200) {
                // articleLabel.innerText = 'Success';
                ...
                const id = data.response_text.post_id;
                delete_test_article(id); 
