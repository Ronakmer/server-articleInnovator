...
async function detail_domain_chart(domain_console_data, domain_analytics_data, domain_article_data, slug_id) {
    try {
        // Resolve all promises in parallel for better performance
        const [consoleData, analyticsData, articleData] = await Promise.all([
            domain_console_data,
            domain_analytics_data,
            domain_article_data
        ...
        const chartElement = document.querySelector("#detail_domain_chart");

        // Destroy existing chart if it exists
        if (window.detail_domain_chart_instance) {
            window.detail_domain_chart_instance.destroy();
        }

        // Initialize new chart
        window.detail_domain_chart_instance = new ApexCharts(chartElement, createChartOptions(chartSeries, formattedDates));
        window.detail_domain_chart_instance.render();
