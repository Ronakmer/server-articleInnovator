...
function set_domain_dropdown(domain_data){
    ...
    domain_data.then((response) => {
        ...
        if (set_domain_id) {
            ...
            const color_input = document.querySelector("input[name='manual_domain_slug_id']");
            color_input.value = set_domain_id;
        }
...
function set_author_dropdown(author_data){
    author_data.then((response) => {
        ...
        if (set_author_id) {
            ...
            // Additionally, set the author input field's value
            const author_input = document.querySelector("input[name='author_slug_id']");
            author_input.value = set_author_id;
        }
...
function set_category_dropdown(category_data) {
    ...
    category_data.then((response) => {
        ...
        setTimeout(() => {
            if (set_category_id && Array.isArray(set_category_id)) {
                console.log(set_category_id, 'r0');
                
                // Iterate over the array of slug_ids directly
                set_category_id.forEach(slug_id => {
...
// tag
function set_tag_dropdown(tag_data, set_tag_id) {
    ...
    tag_data.then((response) => {
        ...
        // set data in update time
        setTimeout(() => {
            if (set_tag_id && Array.isArray(set_tag_id)) {
                console.log(set_tag_id, 'r0');
                
                // Iterate over the array of slug_ids directly
                set_tag_id.forEach(slug_id => {
                    // Find the category by slug_id and select it
                    const category = categories.find(c => c.slug_id == slug_id);
                    if (category) {
                        toggleCategory(category.slug_id, true);  // Mark as selected
