...
// article_traffic_chart chart
async function article_traffic_chart(article_console_data, slug_id) {
    

    article_console_data.then((response) => {
        // console.log(response.metrics_data.aggregates,'88888888888888')
        ...
        if (response && response.metrics_data && response.metrics_data.percentage_data) {
            const trend = response.metrics_data.percentage_data.traffic_change.trend;
            const value = response.metrics_data.percentage_data.traffic_change.value;
        
            const percentageText = document.getElementById(`traffic_percentage_value_${slug_id}`);
            const percentageIcon = document.getElementById(`traffic_percentage_icon_${slug_id}`);
