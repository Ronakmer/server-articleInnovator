...
// article_impressions_chart chart
async function article_impressions_chart(article_console_data, slug_id) {
    article_console_data.then((response) => {
        ...
        if (response && response.metrics_data && response.metrics_data.percentage_data) {
            const impressionsChange = response.metrics_data.percentage_data.impressions_change;
            const trend = impressionsChange.trend;
            const value = impressionsChange.value;
        
            const percentageText = document.getElementById(`impressions_percentage_value_${slug_id}`);
            const percentageIcon = document.getElementById(`impressions_percentage_icon_${slug_id}`);
