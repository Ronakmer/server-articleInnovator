    ...
    try:
        ...
        try:
            if request_user.is_superuser:
                obj = keyword.objects.filter(filters).order_by('-created_date')
            else:
...
def add_keyword(request):
    try:
        ...
        try:
            ...
            return JsonResponse({"error": "workspace found."}, status=404)
        except article_type.DoesNotExist:
            return JsonResponse({"error": "article type found."}, status=404)
        except country.DoesNotExist:
            return JsonResponse({"error": "country found."}, status=404)
        except language.DoesNotExist:
            return JsonResponse({"error": "language found."}, status=404)
...
def update_keyword(request, slug_id):
    try:
        ...
        serialized_data = keyword_serializer(instance=obj, data=data)        
