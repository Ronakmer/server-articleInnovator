...
# def update_manual_article(request_user, request_data):
#     try:
#         data = {
...
#     except Exception as e:
#         print("Unexpected error in update_manual_article:", e)
#         return {"error": "Internal server error.", "success": False}
...
# def update_manual_article(request_user, request_data):
#     try:
#         print(request_data,'ssssssssssssssssssssssssssssssss$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$eeeeeeeeeeeeeeeeeeeeeee')
        
#         article_slug_id = request_data.get("article_slug_id")
#         if not article_slug_id:
#             return {"error": "article_slug_id is required.", "success": False}

#         try:
#             article_obj = article.objects.get(slug_id=article_slug_id)
...
#         if data["category_slug_id"]:
#             category_objs = wp_category.objects.filter(slug_id__in=data["category_slug_id"].split(","))
#             category_ids = list(category_objs)
#             category_names = [c.name for c in category_objs]

#         if data["tag_slug_id"]:
#             tag_objs = wp_tag.objects.filter(slug_id__in=data["tag_slug_id"].split(","))
...
#         article_obj.wp_title = data["wp_title"]
#         article_obj.wp_slug = wp_slug
#         article_obj.article_status = data["article_status"]
#         article_obj.wp_post_id = wp_post_id
#         article_obj.wp_status = data["wp_status"] if data["wp_status"] in dict(article.WP_STATUS_CHOICES) else None
#         article_obj.wp_schedule_time = wp_schedule_time if data["wp_status"] in scheduled_statuses else None
...
#         # Save to S3 and update content path
#         try:
#             s3_data = {
#                 "domain_slug_id": domain_obj.name,
#                 "article_slug_id": article_obj.slug_id,
...
#             return {"error": "Internal server error during S3 save.", "success": False}
...
def update_manual_article(request_user, request_data):
    try:
        ...
        try:
            ...
            # Set default author if not passed
            if not author_slug_id:
                default_author = wp_author.objects.filter(domain_id=domain_obj).first()
                if default_author:
                    author_slug_id = default_author.slug_id
                else:
                    return {"error": "Default author not found.", "success": False}

            # Set default category if not passed
            if not category_slug_id:
                default_cat = wp_category.objects.filter(domain_id=domain_obj, default_section=True).first()
                if default_cat:
                    category_slug_id = default_cat.slug_id
                else:
                    return {"error": "Default category not found.", "success": False}

            # Set default tag if not passed
            if not tag_slug_id:
                default_tag = wp_tag.objects.filter(domain_id=domain_obj).first()
                if default_tag:
                    tag_slug_id = default_tag.slug_id
                else:
                    return {"error": "Default tag not found.", "success": False}
        ...
        if category_slug_id:
            category_slug_id_list = ensure_list(category_slug_id)
            category_objs = wp_category.objects.filter(slug_id__in=category_slug_id_list)
            category_ids = list(category_objs)
            category_names = [c.name for c in category_objs]

        if tag_slug_id:
            tag_slug_id_list = ensure_list(tag_slug_id)
            tag_objs = wp_tag.objects.filter(slug_id__in=tag_slug_id_list)
            tag_ids = list(tag_objs)
            tag_names = [t.name for t in tag_objs]
