...
def list_competitor_seo_extraction_mapping(request):
    try:
        ...
        try:
            obj = competitor_seo_extraction_mapping.objects.filter(filters).order_by('-created_date')
        ...
        return JsonResponse({
            "redirect": "",
            "competitor_seo_extraction_mappings":serialized_data.data,
        }, status=200)
    ...
    try:
        ...
        if serialized_data.is_valid():
            ...
            return JsonResponse({
                ...
                "competitor_seo_extraction_mapping": serialized_data.data,
            }, status=200)
        else:
            return JsonResponse({
                "error": "Invalid data.",
...
def update_competitor_seo_extraction_mapping(request, slug_id):
    try:
        ...
        serialized_data = competitor_seo_extraction_mapping_serializer(instance=obj, data=data)        
