...
def add_test_category(request):
    try:
        ...
        api_url = f'https://{domain_name}/wp-json/wp/v2/categories'
...
@api_view(['POST'])
def delete_test_category(request):
    try:
        
        wp_username = request.data.get('wp_username')
        wp_password = request.data.get('wp_password')
        domain_name = request.data.get('domain_name')  
        test_category_id = request.data.get('test_category_id')  
        
        
        api_url = f'https://{domain_name}/wp-json/wp/v2/categories/{test_category_id}?force=true'
