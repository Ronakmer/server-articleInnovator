...
def list_competitor_extraction(request):
    try:
        ...
        try:
            obj = competitor_extraction.objects.filter(filters, competitor_id=competitor_obj).order_by(order_by)
...
def update_competitor_extraction(request, slug_id):
    try:
        ...
        serialized_data = competitor_extraction_serializer(instance=obj, data=data, partial=True)        
