

import requests
import json 
from django.conf import settings
import os
AI_RATE_LIMITER_BASE_URL = os.getenv("AI_RATE_LIMITER_BASE_URL")



def register_ai_rate_limiter_workspace_api(workspace_slug_id):
    try:

        url = f'{AI_RATE_LIMITER_BASE_URL}/workspace/register'
        print('$$$$$$$$$$$$$$$$$$$$$$$$$$$',url)
        print('$$$$$$$$$$$$$$$$$$$$$$$$$$$',workspace_slug_id)


        headers = {
            'Content-Type': 'application/json',
            # 'Authorization': f'Basic {credentials}',
        }
        data = {
            "workspace_id": workspace_slug_id,
            "providers": []
        }


        response = requests.post(url, headers=headers, json=data)
        print(response.json(),'responsexxxx')
        if response.status_code == 200:
            print('codess',response.status_code)
            return response.json(), None  # Return JSON response
        else:
            print(f"API Error: {response.status_code}, {response.text}")
            return None, f"API Error: {response.status_code}, {response.text}"
            
            
    except Exception as e:
        print("This error is register_ai_rate_limiter_workspace_api --->: ", e)
