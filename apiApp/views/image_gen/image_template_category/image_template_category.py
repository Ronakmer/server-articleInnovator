...
# show image template category
@api_view(['GET'])
def list_image_template_category(request):
    try:
...
# add image template category
@api_view(['POST'])
def add_image_template_category(request):
    try:
        request_user = request.user
        ...
        serialized_data = image_template_category_serializer(data=data)
...
def update_image_template_category(request, slug_id):
    try:
        ...
        try:
            obj = image_template_category.objects.get(slug_id=slug_id)
        except image_template_category.DoesNotExist:
            return JsonResponse({
...
# delete image template category
@api_view(['DELETE'])
def delete_image_template_category(request, slug_id):
    try:
        try:
            obj = image_template_category.objects.get(slug_id=slug_id)
        except image_template_category.DoesNotExist:
