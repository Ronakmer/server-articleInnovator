...
def add_image_tag_template_category_template_mapping(request):
    try:
        ...
        if not (workspace_slug_id and image_tag_slug_id and image_template_category_slug_id and image_template_slug_id):
            return JsonResponse({"error": "workspace, image tag, image template category, image template slug id is required fields.","success": False}, status=400)
        ...
        try:
            ...
            return JsonResponse({"error": "image tag not found.","success": False}, status=404)  
        except image_template_category.DoesNotExist:
            return JsonResponse({"error": "image template category not found.","success": False}, status=404)  
        except image_template.DoesNotExist:
            return JsonResponse({"error": "image template not found.","success": False}, status=404)  
...
def update_image_tag_template_category_template_mapping(request, slug_id):
    try:
        ...
        serialized_data = image_tag_template_category_template_mapping_serializer(instance=obj, data=data, partial=True)        
