...
def list_competitor_article_url_mapping(request):
    try:
        ...
        try:
            ...
            return JsonResponse({"error": "competitor article url not found."}, status=404)
        ...
        try:
            obj = competitor_article_url_mapping.objects.filter(filters).order_by('-created_date')
        except competitor_article_url.DoesNotExist:
            return JsonResponse({
        ...
        return JsonResponse({
            "redirect": "",
            "competitor_article_url_mappings":serialized_data.data,
        }, status=200)
    ...
    try:
        ...
        if serialized_data.is_valid():
            ...
            return JsonResponse({
                ...
                "competitor_article_url_mapping": serialized_data.data,
            }, status=200)
        else:
            return JsonResponse({
                "error": "Invalid data.",
...
def update_competitor_article_url_mapping(request, slug_id):
    try:
        ...
        serialized_data = competitor_article_url_mapping_serializer(instance=obj, data=data)        
