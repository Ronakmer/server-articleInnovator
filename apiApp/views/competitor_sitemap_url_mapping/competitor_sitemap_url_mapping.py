...
def list_competitor_sitemap_url_mapping(request):
    try:
        ...
        try:
            ...
            return JsonResponse({"error": "competitor sitemap url not found.","success": False}, status=404)
        ...
        try:
            obj = competitor_sitemap_url_mapping.objects.filter(filters).order_by(order_by)
...
def update_competitor_sitemap_url_mapping(request, slug_id):
    try:
        ...
        serialized_data = competitor_sitemap_url_mapping_serializer(instance=obj, data=data, partial=True)        
