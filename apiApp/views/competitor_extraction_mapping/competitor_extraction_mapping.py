...
def list_competitor_extraction_mapping(request):
    try:
        ...
        try:
            obj = competitor_extraction_mapping.objects.filter(filters).order_by(order_by)
...
def update_competitor_extraction_mapping(request, slug_id):
    try:
        ...
        serialized_data = competitor_extraction_mapping_serializer(instance=obj, data=data, partial=True)        
