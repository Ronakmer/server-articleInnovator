...
class AccessControlMiddleware(MiddlewareMixin):
    ...
    def __call__(self, request):
        ...
        try:
            ...
            # workspace_ids = user_details_obj.workspace_id.values_list('id', flat=True)
            # workspaces_obj = workspace.objects.filter(id__in=workspace_ids)
            # if not workspaces_obj.exists():
            #     print("No workspace permissions found.")
            #     return JsonResponse({"error": "You don't have access to any workspaces."}, status=403)
